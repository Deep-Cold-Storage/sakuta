  ...
  <v-app>
    ...
    <v-app-bar app color="red accent-3" flat>
      <v-container class="py-1 fill-height">
        ...
        <h2 class="white--text font-weight-medium" v-show="$vuetify.breakpoint.mdAndUp">Create Customer</h2>
      </v-container>
    </v-app-bar>
          ...
          <v-col xs="12" sm="12" md="12" lg="2">
            <v-sheet rounded>
              <v-list rounded>
                ...
                <v-list-item-group>
                  <v-list-item @click="createCustomer">
...
<script>
  export default {
    ...
    methods: {
      createCustomer() {
        this.$http
          .post("/api/contractors", this.customer)
          .then(() => {
            this.$router.go(-1);
          })
          .catch((error) => {
